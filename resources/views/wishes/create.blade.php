...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-purple-600 to-indigo-600 px-8 py-6">
            <h2 class="text-3xl font-bold text-white">✨ {{ __('wishlist.add_wish') }}</h2>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('wishes.store') }}" enctype="multipart/form-data" class="p-8 space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <input type="url" id="wish-url" name="url" value="{{ old('url') }}" placeholder="https://example.com/product" class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
            ...
            <div>
                ...
                <input type="text" id="wish-title" name="title" value="{{ old('title') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
            </div>

            <!-- Description -->
            <div>
                <label for="wish-description" class="block text-sm font-medium text-gray-700 mb-2">{{ __('wishlist.wish_description') }}</label>
                <textarea id="wish-description" name="description" rows="4" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition resize-none">{{ old('description') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    ...
                    <input type="number" id="wish-price" name="price" value="{{ old('price') }}" step="0.01" min="0" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition">
                </div>
